...
| `interrupted` | The app exited while the run was in-flight (`queued` or `running`) |
...
- See context provenance for each run (provider, snippet count, and fallback source when used)
- Distinguish recovered runs with an **`(app exited)`** annotation
...
### A run shows `Interrupted (app exited)`

The app recovered an in-flight run from persisted state during startup. Review the run history and Changes tab, then start a new run from the latest Spec/prompt to continue.
