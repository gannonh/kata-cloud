...
Use `pnpm run test:orchestrator-context` as a targeted regression gate before opening PRs that touch orchestrator lifecycle, persisted run recovery, or context provider behavior. It is also the first command in Phase 8 verification.
...
| `orchestrator-context-diagnostics-uat` | Validate context diagnostics (provider, code, remediation, retryable), history movement, and restart persistence | No | Yes | Yes |
