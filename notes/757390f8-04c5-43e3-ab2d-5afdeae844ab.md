...
- ✅ AC3: Guardrails prevent reintroduction of transient/cache/binary artifact pollution.
  - Commit `fe3e028` adds scoped guardrails (`.gitignore`, `package.json`, `scripts/repo-guardrails.mjs`).
...
- Task 2 (`8de93595-feab-415f-9b77-731a6ac57abc`): evidence is complete and scoped.
  - Commit `fe3e028` touches only `.gitignore`, `package.json`, `scripts/repo-guardrails.mjs`.
...
- Commit scope checks:
  - `git show --name-status --stat --oneline fe3e028` => `.gitignore`, `package.json`, `scripts/repo-guardrails.mjs`
