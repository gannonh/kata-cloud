...
## Changes
- `package.json`
  - Changed `dev` script from fixed `wait-on tcp:5173` orchestration to `node scripts/dev-desktop.mjs`.
- `scripts/dev-desktop.mjs`
...
## Changes (Turn 2)
- Commit: `297fd5a`
- Files in commit: `package.json`, `scripts/dev-desktop.mjs`
...
- Commit scope verified: only `package.json` and `scripts/dev-desktop.mjs`.
