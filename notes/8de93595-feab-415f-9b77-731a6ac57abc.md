...
## Scope
In scope: ignore/validation guardrails for `.pnpm-store`, caches, binaries, and `.DS_Store`; lightweight policy automation in repo bin/config.
...
- ✅ Guardrails flag forbidden artifact classes (`bin/repo-guardrails.mjs`).
...
- `bin/repo-guardrails.mjs` (new): lightweight guardrail checker that scans tracked/staged paths and flags forbidden artifact classes (`.DS_Store`, `.pnpm-store`, `.electron-cache`, `.cache`, `coverage`, `*.tsbuildinfo`).
...
- `package.json`: added `repo:guardrails` script (`node bin/repo-guardrails.mjs`).
