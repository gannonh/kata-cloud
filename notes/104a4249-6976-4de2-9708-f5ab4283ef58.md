...
- Added a targeted temporary suppression for `no-control-regex` in `scripts/dev-desktop.mjs` because its ANSI escape regex is intentional.
...
- `pnpm run lint` initial run (exit `1`): confirmed non-zero behavior on lint errors. Reported errors in `scripts/dev-desktop.mjs` (`no-control-regex`) and TS type-global false positives (`JSX`, `NodeJS`) from `no-undef`.
